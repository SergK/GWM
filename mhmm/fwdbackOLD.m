function [alpha, beta, gamma, loglik, xi_summed, gamma2] = fwdbackOLD(init_state_distrib, ...
...
% normalize beta(t) - the constants will cancel when we compute gamma.
...
for t=2:T
 ...
 if compute_xi & fwd_only  % useful for online EM
   ...
   xi_summed = xi_summed + normalize((alpha(:,t-1) * obslik(:,t)') .* trans);
 end
 assert(approxeq(sum(alpha(:,t)),1))
end
if scaled
...
for t=T-1:-1:1
 ...
 if compute_xi
   ...
   xi_summed = xi_summed + normalize((trans .* (alpha(:,t) * b')));
 end
 if compute_gamma2
   denom = obslik(:,t) + (obslik(:,t)==0); % replace 0s with 1s before dividing
   gamma2(:,:,t) = obslik2(:,:,t) .* mixmat .* repmat(gamma(:,t), [1 M]) ./ repmat(denom, [1 M]);
...
if sum(any(isnan(g)))>0
    warning('gamma2 is NaN in fwdbackOLD');
end
end
