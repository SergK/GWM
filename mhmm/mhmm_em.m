...
if ~iscell(data)
    ...
    %%%%My edition for supportint mhmmFit
elseif length(data)==1
    data = num2cell(cell2mat(data), [1 2]); % each elt of the 3rd dim gets its own cell
end
%numex = length(data);
...
M = size(mixmat,2);     %here we check if we have define M>1
if M == 1               %so if M==1 we don't look at mixmat values
    adj_mix = 0;
end
...
while (num_iter <= max_iter) && ~converged
    ...
    % M step
    if adj_prior
        prior = normalize(exp_num_visits1);
    end
    if adj_trans
        transmat = mkStochastic(exp_num_trans);
    end
    if adj_mix
        mixmat = mkStochastic(postmix);
    end
    if adj_mu || adj_Sigma
    ...
    num_iter =  num_iter + 1;
    showWarning = true;
    converged = convergenceTest(loglik, previous_loglik, thresh,showWarning);
...
% % % % Exp_num_trans: expected number of transitions is the transition matrix
% % % % Exp_num_visits1: expected number of visits to each state is our priors
% % % % Postmix: is the mixing matrix
% % % % M: is the weighted observations
% % % % Ip: is the weighted inner product of the observations
% % % % Op: is the weighted outer product of the observations
...
for ex=1:numex
    ...
    if mix
        ...
        [alpha, beta, gamma,  current_loglik, xi_summed, gamma2] = ...
             fwdbackGamma2(prior, transmat, B, 'obslik2', B2, 'mixmat', mixmat);
    else % we have one gaussian for state
        %% Old style
         B = mixgauss_prob(obs, mu, Sigma);
         [gamma, alpha, beta, current_loglik] = hmmFwdBack(prior', transmat, B);
         xi_summed                   = hmmComputeTwoSliceSum(alpha, beta, transmat,B);
        %% My style
%         logB = nan(Q,numel(obs));
%         for j=1:Q
%             logB(j,:) = gaussLogprob(mu(:, j), Sigma(:, :, j), obs);
%         end
%         [logB, scale] = normalizeLogspace(logB');
%         B             = exp(logB');
%         [gamma, alpha, beta, current_loglik] = hmmFwdBack(prior', transmat, B);
%         xi_summed                   = hmmComputeTwoSliceSum(alpha, beta, transmat,B);
%         current_loglik=current_loglik+sum(scale);
    end
    ...
    if sum(any(isnan(postmix)))>0
        error('postmix is nan');
    end
    for i=1:Q
        for k=1:M
