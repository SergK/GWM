...
while (num_iter <= max_iter) && ~converged
    ...
    if adj_obs
        obsmat = mkStochastic(exp_num_emit);
    end
    
    if verbose, fprintf(1, 'iteration %d, loglik = %f\n', num_iter, loglik); end
    num_iter =  num_iter + 1;
    converged = convergenceTest(loglik, previous_loglik, thresh, true);
...
for ex=1:numex
    ...
    %  [gamma1, alpha1, beta1, logp1] = hmmFwdBack(startprob', transmat, obslik);
    %   xi_summed1                   = hmmComputeTwoSliceSum(alpha1, beta1, transmat, obslik);
